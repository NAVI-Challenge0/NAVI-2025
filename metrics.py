...
def compute_metrics(x):
    ...
    metrics["MRR"] = np.mean(1.0 / (ind + 1))  # rank가 0-based index라 +1 필요
    return metrics
...
def print_computed_metrics(metrics):
    ...
    mr = metrics['MR']
    mrr = metrics['MRR']
    print('R@1: {:.4f} - R@5: {:.4f} - R@10: {:.4f} - Median R: {} - MRR: {:.4f}'.format(r1, r5, r10, mr, mrr))
...
def tensor_text_to_video_metrics(sim_tensor, top_k = [1,5,10]):
    ...
    results['MRR'] = float(torch.mean(1.0 / (valid_ranks + 1)))
    return results
