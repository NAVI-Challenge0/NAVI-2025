...
from dataloaders.dataloader_navi_retrieval import NAVI_DataLoader
...
def dataloader_navi_train(args, tokenizer):
    navi_dataset = NAVI_DataLoader(
        subset="train",
        data_path=args.data_path,
        features_path=args.features_path,
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(navi_dataset)
    dataloader = DataLoader(
        navi_dataset,
        batch_size=args.batch_size // args.n_gpu,
        num_workers=args.num_thread_reader,
    ...
    return dataloader, len(navi_dataset), train_sampler

def dataloader_navi_test(args, tokenizer, subset="test"):
    navi_testset = NAVI_DataLoader(
        subset=subset,
        data_path=args.data_path,
        features_path=args.features_path,
        frame_path = args.frame_path,
    ...
    return dataloader_navi, len(navi_testset)
...
DATALOADER_DICT["navi"] = {"train":dataloader_navi_train, "val":dataloader_navi_test, "test":dataloader_navi_test}
